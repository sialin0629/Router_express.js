...
dotenv.config(); // dotenv 설정 파일 불러오기
...
nunjucks.configure('views', {
  ...
  express: app,
  // Nunjucks가 Express의 템플릿 엔진으로 사용될때, Express 애플리케이션과 연동될 수 있도록 설정
  watch: true,
  // 템플릿 파일의 변경 -> Nunjucks 자동업데이트 수행
  // 템플릿 파일이 변경되면 변경사항을 서버 재시작 없이 반영
});

// morgan 미들웨어 -> 개발 환경에서 요청로그 출력
app.use(morgan('dev'));
...
app.use((err, req, res, next) => {
  ...
  // process.env.NODE_ENV -> 현재 실행 환경을 나타내는 Node.js의 환경변수
  // 에러 객체를 'production'(배포) 환경이 아닌 경우(개발환경 또는 테스트 환경)에만 로컬 변수에 할당
  // 'production'(배포) 환경에서는 빈 개게 할당 -> 보안 및 개인정보 강화(error.html의 error.stack 노출 차단)
  res.status(err.status || 500);
  // 응답 상태 코드를 에러 객체의 상태 코드 또는 500으로 설정
  res.render('error')
  // 'error' 템플릿을 렌더링 하여 에러 페이지에 표시
});
